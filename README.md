...
| routing_log says assigned but the ticket shows unassigned | racing writes to one row **from different connections** can shadow each other when the warehouse write-log drains — even after a STRONG read confirmed the write (verified live, twice; same-connection sequential writes are safe). The engine re-checks past the drain window and re-asserts if the field reverted (`route_ticket` in [`actions/routing.py`](actions/routing.py)); do the same for any cross-writer row you add. |
